...
Global styles: if you add a `<style>` or `<link>` tag with the `melodic-styles` attribute, Melodic will share it across components. The attribute name is fixed. For Vite builds, link a file under `/src` (for example `/src/styles/global.css`) to get minified, hashed output; link a public asset (for example `/styles/global.css`) if you want to preserve the path.
