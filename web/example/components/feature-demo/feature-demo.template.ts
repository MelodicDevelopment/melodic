...
export function featureDemoTemplate(component: FeatureDemoComponent) {
	return html`
		<div class="container">
			...
			<test-component .title=${`Just A Test ${component.count()}`} .label=${`Passed from Demo (count: ${component.count()})`}></test-component>
					...
					${repeat(
						...
						(todo) => html`
							...
							>
								...
								<button @click=${() => component.removeTodo(todo.id)} class="delete-btn">X</button>
							</li>
						`
					)}
				</ul>
